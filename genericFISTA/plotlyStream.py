...
class JupyterNotebookPlotlyStream:
    ...
    def sealUpdatedFigure(self):
        """
        Make the update permanant after the widget is not present.

        Previously, after the notebook is reopened, the latest plot disappears because the widget
        is cleared. This function is to make the last figure last.

        It is recommanded to call this method within the same cell that invokes update().
        """
        plot_js = self._widget_JS.value
        IPython.display.display(IPython.display.Javascript(plot_js))
